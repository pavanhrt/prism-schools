...
import { schoolSettingsUpdateSchema, websiteFeatureSchema, websiteProgramSchema, websiteRecordIdSchema, websiteServiceSchema, type SchoolSettingsUpdateInput, type WebsiteFeatureInput, type WebsiteProgramInput, type WebsiteServiceInput } from "@/validations/settings";
import * as service from "./service";

export type ActionResult = { ok: true } | { ok: false; error: string };
const failure = (error: unknown, fallback: string): ActionResult => ({ ok: false, error: error instanceof Error ? error.message : fallback });
