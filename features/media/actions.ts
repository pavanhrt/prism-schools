...
import { entityIdSchema, galleryItemIdSchema, galleryItemSchema, galleryItemUpdateSchema, type GalleryItemUpdateInput } from "@/validations/media";
...
export async function deleteGalleryItemAction(id: string): Promise<GalleryActionResult> {
  try { await requirePermission("website_settings.manage"); await service.deleteGalleryItem(await createClient(), galleryItemIdSchema.parse(id)); refreshPublicWebsite(); revalidatePath("/gallery"); return { ok: true }; }
