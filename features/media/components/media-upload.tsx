...
import { removePublicMediaAction, uploadPublicMediaAction } from "@/features/media/actions";
...
export function MediaUpload({ category, entityId = null, current, label }: { category: string; entityId?: string | null; current: string | null; label: string }) {
  ...
  return <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
    ...
    {message && <p aria-live="polite" className={`mt-2 text-xs ${message === "Uploaded and saved." || message === "Removed successfully." ? "text-emerald-700" : "text-red-600"}`}>{message}</p>}
  </div>;
}
