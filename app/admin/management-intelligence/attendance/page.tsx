...
export default async function AttendanceIntelligencePage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const selectedStudent = students.find((student) => student.id === params.student_id);
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card><CardContent>
        <form className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">
          ...
          <label className="text-xs font-medium text-slate-600">Student<select name="student_id" defaultValue={params.student_id ?? ""} className="mt-1 h-9 w-full rounded-md border border-slate-300 px-2 text-sm"><option value="">All students</option>{students.map((student) => <option key={student.id} value={student.id}>{student.name} · {student.admissionNo}</option>)}</select></label>
      ...
      {params.student_id && <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-900"><span>{selectedStudent ? <>Showing <strong>{selectedStudent.name}</strong> ({selectedStudent.admissionNo}) · {selectedStudent.className} {selectedStudent.sectionName}</> : <>The selected student is not enrolled in this academic year or is not available to your role.</>}</span><Link href="/admin/management-intelligence/attendance" className="font-medium hover:underline">Clear student filter</Link></div>}
